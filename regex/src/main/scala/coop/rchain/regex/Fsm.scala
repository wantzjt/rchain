...
private[regex] object StateActionType extends Enumeration {
  val cancelGroup, completeGroup, enterGroup = Value
}
...
//  */
//case class StateAction(priority: Int,
//                       actionType: StateActionType.Value,
//                       action: (MatchData, Int) => (MatchData, Option[Int]))

/**
  * We can bind a call-back functions to any Fsm state. These functions will be called
  * in order specified by the priority
  */
case class TransitionAction(actionType: StateActionType.Value,
                            action: (MatchData, Int) => (MatchData, Option[Int]))

case class Transition(nextState: Int, actions: List[TransitionAction])
...
object Fsm {
  ...
  def nullFsm(alphabet: Set[Char]): Fsm =
    Fsm(alphabet, Set(0), 0, Set(), Map(0 -> alphabet.map(_ -> Transition(0, Nil)).toMap))

  /**
    * Return an FSM matching an empty string, "", only.
    */
  def epsilonFsm(alphabet: Set[Char]): Fsm =
    Fsm(alphabet, Set(0), 0, Set(0), Map[Int, Map[Char, Transition]]())
  ...
  private[regex] def crawl[T](alphabet: Set[Char],
                              initial: T,
                              isFinal: T => Boolean,
                              follow: (T, Char) => Option[(T, List[TransitionAction])]): Fsm = {
    //actual type of the 'states' is deducted from 'initial' => ArrayBuffer[T]
    val states         = mutable.ArrayBuffer[(T, List[TransitionAction])]((initial, Nil))
    val transitions    = mutable.Map[Int, Map[Char, Transition]]()
    ...
    def iterate(currentStateIdx: Int = 0, currentFinals: Set[Int] = Set()): Set[Int] = {
      val (currentState, _) = states(currentStateIdx)
        ...
        .flatMap(symbol =>
          follow(currentState, symbol).map(nextState => {
            ...
            symbol -> (if (nextStateIdx < 0) {
                         states += nextState
                         Transition(states.size - 1, nextState._2)
                       } else {
                         Transition(nextStateIdx, states(nextStateIdx)._2)
                       })
          }))
        .toMap
  ...
  def parallel(finalityTest: Seq[Boolean] => Boolean, fsms: Seq[Fsm]): Fsm = {
    ...
    def follow(currentState: Map[Int, Int],
               symbol: Char): Option[(Map[Int, Int], List[TransitionAction])] = {
      val next = fsms.zipWithIndex.flatMap {
        case (fsm, fsmIdx) =>
          currentState
            ...
            .flatMap(
              fsm
                .nextTransition(_, symbol)
                .map(nextState => (fsmIdx -> nextState.nextState, nextState.actions)))
      }

      val nextMap = next.map(_._1).toMap
      if (nextMap.nonEmpty)
        Some((nextMap, next.flatMap(_._2).toList.flatten))
      else
        None
    }
  ...
  def concatenate(fsms: Fsm*): Fsm = {
    ...
    def follow(currentStates: Set[(Int, Int)],
               currentSymbol: Char): Option[(Set[(Int, Int)], List[TransitionAction])] = {
      val next = currentStates.toList.flatMap {
        case (fsmIndex, fsmState) => {
          val fsm = fsms(fsmIndex)
          fsm
            .nextTransition(fsmState, currentSymbol)
            .map(nextState => (connectAll(fsmIndex, nextState.nextState), nextState.actions))
        }
      }

      val nextStates = next.flatMap(_._1)
      if (nextStates.nonEmpty) {
        Some((nextStates.toSet, next.flatMap(_._2).flatten))
      } else {
        None
      }
    }
  ...
  /**
    ...
    */
  private[regex] val actionsOrdering = Ordering.by((_: TransitionAction).actionType)

	def apply(alphabet: Set[Char],
						states: Set[Int],
						initialState: Int,
						finalStates: Set[Int],
						transitions: Map[Int, Map[Char, Int]]): Fsm = {

			new Fsm(alphabet, states, initialState, finalStates, transitions.map {
				case (state, transition) => state -> transition.map {
					case (c, nextState) => c -> Transition(nextState, Nil)
				}
			})
	}
}
...
case class Fsm(alphabet: Set[Char],
               ...
               initialState: Int,
               finalStates: Set[Int],
               transitions: Map[Int, Map[Char, Transition]]) {
  ...
  for {
    ...
    require(states.contains(nextState.nextState),
  ...
  def bindActions(enterAction: TransitionAction,
                  leaveAction: TransitionAction,
                  cancelAction: TransitionAction): Fsm =
...
//    newActionsMap ++= boundActions.map {
//      case ((state1, state2), actions) => (state1, state2) -> actions
//    }
//
...
//        transitionsFromInitial.map(_._2).filter(isLive).map(x => (initialState, x))
...
//        (finalState, -1) :: transitions
//          .getOrElse(finalState, Nil)
//          .toList
//          .filter(c => !isLive(c._2))
//          .map {
...
//      val transitionsOutside = transitions
//        .filter(transition => !finalStates.contains(transition._1))
//        .flatMap {
...
//    }
    Fsm(alphabet, states, initialState, finalStates, transitions)
  ...
  def isLive(state: Int): Boolean = {
    ...
    while (reachable.nonEmpty) {
      ...
      reachable ++= transitions
        .get(reachableState)
        .map(_.values)
        .flatMap(_.filter(transitionTarget => !checked.contains(transitionTarget.nextState))
          .map(_.nextState))
    }
  ...
  def matchIn(initalMatchData: MatchData): (MatchData, Int, Option[Int]) = {
    ...
    def processActions(matchData: MatchData,
                       charIndex: Int,
                       minDiff: Int,
                       lst: List[TransitionAction]): (MatchData, Int) =
      lst match {
        case head :: tail =>
    ...
    def acceptChar(currentState: Int,
      ...
      if (charIndex < input.length) {
        ...
        nextTransition(currentState, sym) match {
          ...
          case Some(nextState) =>
            ...
            val (leaveMatchData, leaveDiff) = processActions(
              ...
              charIndex,
              0,
              boundActions.getOrElse((currentState, nextState), Nil).sorted(Fsm.actionsOrdering))
        ...
        val (leaveMatchData, leaveDiff) = processActions(
          ...
          charIndex,
          0,
          boundActions.getOrElse((currentState, -1), Nil).sorted(Fsm.actionsOrdering))
  ...
  def derive(input: String): Try[Fsm] = {
    ...
    for (currentSymbol <- input) {
      ...
      nextTransition(currentState, sym) match {
        case Some(transition) => currentState = transition.nextState
  ...
  def reversed: Fsm = {
    ...
    def follow(currentStates: Set[Int],
               currentSymbol: Char): Option[(Set[Int], List[TransitionAction])] = {
      val nextStates = (for {
        (transitionState, transitionMap) <- transitions
        currentState                     <- currentStates
        if transitionMap.get(currentSymbol).map(_.nextState).contains(currentState)
  ...
  def everythingBut: Fsm = {
    ...
    def follow(currentState: List[Int],
               currentSymbol: Char): Option[(List[Int], List[TransitionAction])] = {
      val next = currentState.headOption
        .map(headState => {
          val stNextOpt  = nextTransition(headState, currentSymbol)
          val nextAction = stNextOpt.flatMap(stNext => boundActions.get(headState, stNext))
          (stNextOpt, nextAction)
        })

      if (next.nonEmpty) {
        Some(next.get._1.toList, next.get._2.getOrElse(Nil))
      } else {
        None
      }
    }
      ...
      if (pendingFinals.nonEmpty) {
        ...
        while (pendingFinals.isEmpty && pending.nonEmpty) {
          ...
          transitions
            ...
            .foreach({
              case (nextSymbol, nextTransition) => {
                val nextState  = nextTransition.nextState
  ...
  def times(multiplier: Int): Fsm = {
    ...
    def follow(crawlState: Set[(Int, Int)],
               symbol: Char): Option[(Set[(Int, Int)], List[TransitionAction])] = {
      val next = crawlState.toList
        .filter { case (_, iteration) => iteration < multiplier }
        .flatMap {
          case (fsmState, iteration) =>
            nextTransition(fsmState, symbol).map(subState =>
              if (finalStates.contains(subState.nextState)) {
                (List(subState.nextState -> iteration, initialState -> (iteration + 1)),
                 subState.actions)
              } else {
                (List(subState.nextState -> iteration), subState.actions)
            })
        }

      if (next.nonEmpty) {
        Some((next.flatMap(_._1).toSet, next.flatMap(_._2).flatten))
      } else {
        None
      }
    }
  ...
  /**
    ...
    */
  def nextTransition(state: Int, symbol: Char): Option[Transition] =
  ...
  def star: Fsm = {
    ...
    def follow(subStates: Set[Int], symbol: Char): Option[Set[Int]] = {
      ...
      val next = subStates
        .flatMap(fsmState =>
          if (finalStates.contains(fsmState)) {
            List(nextState(fsmState, symbol), nextState(initialState, symbol))
          } else {
            List(nextState(fsmState, symbol))
        })
        .flatten
    ...
    def follow(subStates: Set[Int], symbol: Char): Option[(Set[Int], List[TransitionAction])] = {
      val next = subStates.toList
        .map(fsmState => {
          val transitionPair =
            (if (finalStates.contains(fsmState)) {
               List(nextTransition(fsmState, symbol), nextTransition(initialState, symbol))
             } else {
               List(nextTransition(fsmState, symbol))
             }).flatten
          (transitionPair.map(_.nextState).toSet, transitionPair.flatMap(_.actions))
        })
    ...
    Fsm.crawl(alphabet, Set(initialState), isFinal, follow) | Fsm.epsilonFsm(alphabet)
  }
          ...
          stateCount
            .getOrElse({
              ...
              val n = (if (finalStates.contains(state)) {
                ...
                .map(transition =>
                  //yup, recursion here
                  getNumStrings(transition.nextState).getOrElse({
  ...
  /**
    ...
    */
  def reduced: Fsm = this //reversed.reversed
