...
trait ConcurrencyTests extends StorageTestsBase[Channel, Pattern, Entry, EntriesCaptor] {
  ...
  "CORE-589 produce and consume in a multithreaded fashion" should
    ...
    println(
      s"${counter.producesCount} produces, avg ${counter.produceAvgMilliseconds.formatted("%.2f")} ms per produce")
    println(
      s"${counter.consumesCount} consumes, avg ${counter.consumeAvgMilliseconds.formatted("%.2f")} ms per consume")
