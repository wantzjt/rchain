...
class InMemoryStore[T, C, P, A, K](
  ...
  protected override def processTrieUpdate(cacheStore: TrieStoreType, txn: TrieTransaction, update: TrieUpdate[C, P, A, K]): Unit =
    update match {
      case TrieUpdate(_, Insert, channelsHash, gnat) =>
        history.insert(cacheStore, txn, trieBranch, channelsHash, canonicalize(gnat))
      case TrieUpdate(_, Delete, channelsHash, gnat) =>
        history.delete(cacheStore, txn, trieBranch, channelsHash, canonicalize(gnat))
    }
