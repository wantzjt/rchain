...
package object rspace {
  ...
  /** Iterates through (channel, pattern) pairs looking for matching data.
		...
		*/
  @tailrec
  private[rspace] final def extractDataCandidates[C, P, A](
  ...
  /** Searches the store for data matching all the given patterns at the given channels.
		...
		* @tparam F a type constructor representing an effect
		* @tparam C A type representing a channel
		* @tparam P A type representing a pattern
		* @tparam A A type representing a piece of data
		* @tparam K A type representing a continuation
		*/
  def consume[F[_], C, P, A, K](channels: Seq[C],
                                patterns: Seq[P],
                                continuation: K,
                                persist: Boolean)(implicit matchPA: Match[P, A],
                                                  store: Store[F, C, P, A, K],
                                                  monadF: Monad[F]): F[Option[(K, Seq[A])]] = {
    ...
    store.createTxnWrite().flatMap { txn =>
      store.withTxn(txn) { txn =>
        ...
        /*
			 ...
			 */
        val channelToIndexedData = channels
          .traverse { channel =>
            for (data <- store.getData(txn, Seq(channel)))
              yield channel -> Random.shuffle(data.zipWithIndex)
          }
          .map(_.toMap)

        val monadOptions = for {
          channelToIndexedMap <- channelToIndexedData
        } yield extractDataCandidates(channels.zip(patterns), channelToIndexedMap, Nil)

        val options: F[Option[Seq[DataCandidate[C, A]]]] = monadOptions.map(_.sequence)
        ...
        options.map {
          case None =>
            ...
            for (channel <- channels)
              store.putJoin(txn, channel, channels)
  ...
  def install[F[_], C, P, A, K](channels: Seq[C], patterns: Seq[P], continuation: K)(
      implicit matchPA: Match[P, A],
      store: Store[F, C, P, A, K],
      monadF: Monad[F]): F[Option[(K, Seq[A])]] = {
    if (channels.length =!= patterns.length) {
      val msg = "channels.length must equal patterns.length"
      ...
      store.withTxn(txn) { txn =>
        ...
        options.map {
          case None =>
            ...
            for (channel <- channels) store.putJoin(txn, channel, channels)
  ...
  /** Searches the store for a continuation that has patterns that match the given data at the
		...
		* @tparam K A type representing a continuation
		*/
  def produce[F[_], C, P, A, K](channel: C, data: A, persist: Boolean)(
      implicit matchPA: Match[P, A],
      store: Store[F, C, P, A, K],
      monadF: Monad[F]): F[Option[(K, Seq[A])]] =
    store.createTxnWrite().flatMap { txn =>
      store.withTxn(txn) { txn =>
        ...
        def extractProduceCandidate(groupedChannels: Seq[Seq[C]],
                                    batChannel: C,
                                    data: Datum[A]): F[Option[ProduceCandidate[C, P, A, K]]] =
          groupedChannels match {
            case Nil                   => monadF.pure(None)
            case channels :: remaining =>
              /*
							 ...
							 */
              val channelToIndexMonad: F[Map[C, Seq[(Datum[A], Int)]]] = channels
                .traverse { (channel: C) =>
                  for {
                    storedData <- store.getData(txn, Seq(channel))
                  } yield {
                    val shuffledData: Seq[(Datum[A], Int)] = Random.shuffle(storedData.zipWithIndex)
                    channel -> (if (channel == batChannel)
                                  (data, -1) +: shuffledData
                                else
                                  shuffledData)
                  }
              ...
              (for {
                channelToIndexedData <- channelToIndexMonad
                matchCandidates      <- store.getWaitingContinuation(txn, channels).map(_.zipWithIndex)
              } yield {
                extractFirstMatch(channels, Random.shuffle(matchCandidates), channelToIndexedData) match {
                  ...
                  case produceCandidate => monadF.pure(produceCandidate)
                }
              }).flatten
          }

        val produceCandidate = for {
          groupedChannels <- store.getJoin(txn, channel)
        } yield {
